    ...
    >
      ...
      >
        ...
        <template class="d-flex align-items-center" v-if="(picker.startDate.toString() == picker.endDate.toString())">
          <template>{{ new Date(data_value.startDate) | singledate }}</template>
        </template>
        <template
          v-else
        >{{ new Date(data_value.startDate) | date }} - {{ new Date(data_value.endDate) | date }}</template>
      </div>
...
export default {
  ...
  methods: {
    conver_epoch_to_UTC: function () {
      if (this.epoch_format && !new Date(this.data_value.startDate).getDate()) {
        var start = new Date(this.data_value.startDate * 1000);
        var end = new Date(this.data_value.endDate * 1000);
        this.data_value.startDate =
          start.getFullYear() + '-' + start.getMonth() + '-' + start.getDate();
        this.data_value.endDate =
     ...
     * @function format_tooltip_date - retrieves the date for tooltip from date_range_object
     * @param date_range_object
     */
    // eslint-disable-next-line camelcase
    format_tooltip_date (date_range_object) {
      // This.tooltip_year.startYear = moment(date_range_object.startDate).format(
      //   "YYYY"
      // );
      // This.tooltip_year.endYear = moment(date_range_object.endDate).format(
      //   "YYYY"
      // );
      if (date_range_object.startDate) {
        this.tooltip_year.startYear = new Date(
          date_range_object.startDate
        ).toDateString();
      }
      if (date_range_object.endDate) {
        this.tooltip_year.endYear = new Date(
          date_range_object.endDate
        ).toDateString();
      }
    },
    ...
    update_value (values) {
      ...
      if (this.epoch_format) {
        var dataValue = JSON.parse(JSON.stringify(this.data_value));

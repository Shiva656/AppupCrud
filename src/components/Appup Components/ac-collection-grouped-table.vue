<template>
  <div class="ac-table">
        ...
        <table class="table border">
          ...
          <tbody v-for="(value, key) in actual_data" :key="key">
            <tr>
              ...
              <td :colspan="col_span" class="align-middle grouped-category">
                <!-- Use this slot group_header for each item in response -->
                <slot name="group_header" :group-item="value[0]"></slot>
              </td>
            </tr>
            <template v-if="value[0][empty_group_key] == null && empty_group_state">
              <tr>
                ...
                <tr v-for="(innerObject, key) in value" :key="key">
                  ...
                  <template v-for="(column, index) in columns">
                    <td
                      scope="col"
                      class="align-middle"
                      @click="$emit('row-clicked',innerObject)"
                      @dblclick="$emit('row-dblclicked',innerObject)"
                      :key="index"
                    >
                  ...
                  <td scope="col" class="align-middle" v-if="dropdown">
                    <slot name="option" :data="{item:innerObject,index:key}">
                      <ac-material-dropdown
                        ...
                        @item_click="on_dropdown_item_click($event, innerObject,key)"
                      />
                    </slot>
                  </td>
    ...
    <div v-if="!Object.keys(actual_data).length && data_loaded == true && !is_loading">
...
export default {
  ...
  computed: {
    ...
    table_columns () {
      ...
      if (this.dropdown) {
        ...
        if (optionIndex === -1) {
          const obj = { key: 'options', label: 'Options' }
          cols.push(obj);
        }
      }
  ...
  watch: {
    ...
    actual_data () {
      ...
      var groupedKeys = Object.keys(this.actual_data);
      for (var i = 0; i < groupedKeys.length; i++) {
        this.group_form[groupedKeys[i]] = false;
      }
    },
      ...
      if (this.grouped_bulk) {
        ...
        if (Object.keys(selectedGroup).length > 0) {
          for (var key in selectedGroup) {
            if (selectedGroup[key].length === this.actual_data[key].length) {
  ...
  methods: {
     ...
     * @param {array} changeData - grouped data
     */
    on_change (event, changeData) {
      const index = event.moved.newIndex;

      if (index !== 0 && index + 1 !== Object.keys(changeData).length) {
        this.dragged_ids.from_id = changeData[index - 1].id;
        this.dragged_ids.to_id = changeData[index + 1].id;
      } else if (index === 0) {
        this.dragged_ids.to_id = 0;
        this.dragged_ids.from_id = changeData[index + 1].id;
      } else if (index + 1 === Object.keys(changeData).length) {
        this.dragged_ids.to_id = changeData[index - 1].id;
    ...
    bulk_actions (target) {
      ...
      if (!this.select_all || target) {
        ...
        for (const key in this.actual_data) {
